...
Create a new environment for these lessons, which will include the [GNU Compilers](https://gcc.gnu.org/) and an implementation of MPI, [MPICH](https://www.mpich.org/).

**$ conda create --name sss_parallel
$ source activate sss_parallel
$ conda install -c conda-forge gcc mpich**

Acquire a copy of the example files for this lesson, then build and compile the first example:
...
There are a few components that are necessary for any MPI code.
1. The code must `#include <mpi.h>`.
2. As early in the code as possible, it must initialize MPI:

```c++
  MPI_Init(&argc,&argv);
```

This must happen before any other MPI functions are called.  The `argc` and `argv` variables contain arguments entered by the user at the command-line.
More specifically, `argc` should contain the number of command-line arguments entered by the user, while `argv` contains the actual arguments.
Your `main` function can accept this information if you add `argc` and `argv` to its parameter list:

```c++
int main(int argc, char** argv) {
```

3. As close to the end of the code as possible, it must finalize MPI:

```c++
  MPI_Finalize();
```

If you do the above, your code should look like:
